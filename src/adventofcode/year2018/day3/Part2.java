...
public class Part2 {
	...
	public static void main(String[] args) {
		...
		ArrayList<Integer> claimNumbersAll = new ArrayList<Integer>();
		ArrayList<Integer> claimNumbersWithOverlap = new ArrayList<Integer>();
		...
		for (int i = 0; i < args.length; i+=4) {
			...
			claimNumbersAll.add(claimNumber);
		...
		for(Square square:squares.values()){
			if(square.getClaimNumbers()!=null && square.getClaimNumbers().size()>1){
				...
				claimNumbersWithOverlap.addAll(square.getClaimNumbers());
			}
		}
		
		claimNumbersAll.removeAll(claimNumbersWithOverlap);
		
		System.out.println("Overlapping Square Count :" + multipleClaimsPerSquareCount);
		System.out.print("Claim Numbers without Overlap :");
		for (int i = 0; i<claimNumbersAll.size(); i++) {
			System.out.print(claimNumbersAll.get(i));
			System.out.print(" ");
		}
